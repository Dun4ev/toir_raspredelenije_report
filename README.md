...
## Основные каталоги
- `INBOX_DIR` — входной каталог вида `/ГОД/ПапкаПроекта`; внутри каждой подпапки должен находиться **только один** `_All.pdf`. Если положить несколько файлов, будет обработан лишь первый.
...
## Схема работы
1. Входной `_All` файл помещается в `INBOX_DIR`.
...
- Перейти от плоского обхода `00_Inbox` к рекурсивному поиску `_All.pdf` и определить, что считать `project_path` (папка с файлом или ближайший родитель).
...
- Нужна ли поддержка произвольной глубины вложенности или достаточно фиксированного количества уровней?
